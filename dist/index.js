...
async function action(octokit, request) {
    ...
    const metadataFile = (0,core.getInput)('metadata-file-name');
    (0,core.setOutput)('metadata', metadataJson);
    (0,core.setOutput)('metadata-file', metadataFile);
    (0,external_fs_.writeFile)(`./${metadataFile}`, metadataJson, err => {
        if (err) {
            throw new Error(`Failed to write Pull Request metadata into ${metadataFile} file ; fs.writeFile: ${err}`);
        }
        (0,core.debug)(`Successfully wrote Pull Request metadata into a ${metadataFile} file`);
    });
}
...
const prNumber = +(0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput)('pr-number', { required: true });
const metadataFileName = (0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput)('metadata-file-name', { required: true });
const owner = zod__WEBPACK_IMPORTED_MODULE_3__.z.string()
    .min(1)
    .parse((_a = process.env.GITHUB_REPOSITORY) === null || _a === void 0 ? void 0 : _a.split('/')[0]);
const repo = zod__WEBPACK_IMPORTED_MODULE_3__.z.string()
    .min(1)
    .parse((_b = process.env.GITHUB_REPOSITORY) === null || _b === void 0 ? void 0 : _b.split('/')[1]);
try {
    await (0,_action__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .Z)(octokit, { owner, repo, pull_number: prNumber });
}
catch (error) {
    let message;
